...
export default function EnhancedTable() {
  ...
  const [rows, setRows] = useState(dataResult);
  ...
  const searchFunction = (value) => {
    ...
    const results = dataResult.filter(
      (x) => x.title.search(input) !== -1 ||
      x.category.search(input) !== -1 ||
      x.pubDate.search(input) !== -1 ||
      // x.price.search(input) !== -1 ||
      // x.size.search(input) !== -1 ||
      x.link.search(input) !== -1 ||
      x.source.search(input) !== -1
    );
...
// const results = my_list.filter (x => ((x.username.search({value})!=-1) || (x.email.search(value)!=-1)) || (x.skills.search(value)!=-1));
// console.log(results);
  }
